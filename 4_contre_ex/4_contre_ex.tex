...
%   \suchthat \alpha \equ \bigwedge_{i = 1}^n \alpha_i \land \alpha'$$
  ...
  Le couple $(\alpha', q')$ est dans l'ensemble $\{ (q_k, \alpha_k) \sep q_k \xrw{\alpha_k}^* q\}_{k=1}^n$.%_1^m$.
...
Intuitivement, si on a $u \xrw{=}^*_{A} q$, alors on sait qu'il existe
...
Soit $\aaexeq^k$ %= \la \F, \Q^k, \Q_f, \Delta^k \cup \Drw^k \cup \Deq^k\ra
...
\begin{definition}[Élagage d'un $\RE$-automate]
...
Si  $\phi= \bigwedge_1^n Eq(q_j, q'_j)$, on élague le
...
classes de systèmes de réécriture qui terminent. %$\R$ où $\R$ termine.
%pour lesquels $\desc(\Lang(\A))$ est un langage régulier. 
...
Pour donner une idée des conséquences, on peut considérer une séquence de $\varepsilon$-transitions
